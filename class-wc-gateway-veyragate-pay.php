...
class WC_Gateway_VeyraGate_Pay extends WC_Payment_Gateway {
	...
	/**
	 * Mini-cart drawer (0.7.0) — return a JSON snapshot of the live cart
	 * for the slide-out drawer. READ-ONLY: it reads WC()->cart and never
	 * mutates it, so it cannot affect the cart, checkout, or any charge.
	 * Nonce-guarded (the same express nonce). On any error it returns an
	 * empty-but-valid payload so the drawer degrades to a "open full cart"
	 * link rather than breaking.
	 */
	public function handle_minicart_contents_ajax() {
		if ( ! check_ajax_referer( 'veyragate_pay_express', 'nonce', false ) ) {
			wp_send_json_error( array( 'reason' => 'bad_nonce' ), 403 );
			return;
		}
		...
		try {
			...
			foreach ( $cart->get_cart() as $ci ) {
				if ( empty( $ci['data'] ) || ! is_object( $ci['data'] ) ) {
					continue;
				}
				$product = $ci['data'];
				...
				$items[] = array(
					...
					'qty'       => $qty,
					'line_html' => $cart->get_product_subtotal( $product, $qty ),
	...
	public function enqueue_minicart_assets() {
		...
		wp_localize_script(
			...
			array(
				...
				'expressConfigAjaxUrl' => admin_url( 'admin-ajax.php' ),
				'expressPayAjaxUrl'    => admin_url( 'admin-ajax.php' ),
				'cartContentsAjaxUrl'  => admin_url( 'admin-ajax.php' ),
				'expressNonce'         => wp_create_nonce( 'veyragate_pay_express' ),
