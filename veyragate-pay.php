...
define( 'VEYRAGATE_PAY_VERSION', '0.7.0' );
...
function veyragate_pay_init() {
	...
	// Mini-cart drawer (0.7.0) — returns the live cart (items + subtotal +
	// count + cart/checkout URLs) as JSON for the slide-out drawer.
	// Read-only: reads WC()->cart, never mutates it.
	add_action( 'wp_ajax_veyragate_pay_minicart_contents', 'veyragate_pay_minicart_contents_endpoint' );
	add_action( 'wp_ajax_nopriv_veyragate_pay_minicart_contents', 'veyragate_pay_minicart_contents_endpoint' );

	add_action( 'admin_notices', 'veyragate_pay_csp_admin_notice' );
}

/**
 * Mini-cart drawer — cart-contents endpoint (0.7.0). Delegates to the
 * gateway's handle_minicart_contents_ajax(), which reads WC()->cart and
 * returns a JSON snapshot for the drawer. Never mutates the cart.
 */
function veyragate_pay_minicart_contents_endpoint() {
	if ( ! class_exists( 'WC_Gateway_VeyraGate_Pay' ) ) {
		require_once VEYRAGATE_PAY_PLUGIN_PATH . 'includes/class-wc-gateway-veyragate-pay.php';
	}
	$gateway = new WC_Gateway_VeyraGate_Pay();
	$gateway->handle_minicart_contents_ajax();
}
