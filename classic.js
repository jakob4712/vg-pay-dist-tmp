...
(function () {
  ...
  // Apple Pay mark — the same glyph the mini-cart drawer and the Next.js
  // storefronts (evolabs / evobones / axis) render. `currentColor` so it
  // inherits the button's text color (white on the black Apple button).
  // Google Pay is disabled on this inline WooCommerce lane — see the long
  // note at the bootstrap's step 5. Apple-Pay-only until the GP iframe +
  // Google production approval land. Flip to true ONLY together with those.
  var WC_GOOGLE_PAY_ENABLED = false;

  var APPLE_PAY_MARK_SVG =
    ...
    "</svg>";
  // Official Google "G" 4-colour mark, paired with "Pay".
  var GOOGLE_PAY_MARK_SVG =
    '<svg class="veyragate-pay-wallet__mark" width="18" height="18" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" focusable="false">' +
  ...
  /**
   * Render a wallet button container above the card iframe. Buttons use the
   * official Apple Pay / Google Pay marks + "Pay", matching the house style
   * shared by the mini-cart drawer and the Next.js storefronts (evolabs /
   * evobones / axis): a 52px black Apple Pay button and a white Google Pay
   * button, SF-Pro 18/500, 10px radius. The marks are inline SVG (no extra
   * network asset, no CSP change). Apple renders only on Apple-capable
   * browsers (isAppleWalletAvailable); Google only once isReadyToPay resolved.
   */
  function renderWalletButtons(rootEl, cfg, onWalletClick) {
    ...
    if (isAppleWalletAvailable() && cfg.apple_pay_enabled) {
      ...
      apple.setAttribute("aria-label", "Pay with Apple Pay");
      apple.innerHTML = APPLE_PAY_MARK_SVG + "<span>Pay</span>";
    ...
    if (
      ...
      google.setAttribute("aria-label", "Pay with Google Pay");
      google.innerHTML = GOOGLE_PAY_MARK_SVG + "<span>Pay</span>";
  ...
  function tryBootstrapWallets() {
    ...
    postExpressAjax(cfg.expressOrderAjaxUrl, {
      ...
      .then(function (walletCfg) {
        ...
        // 5. Google Pay is INTENTIONALLY DISABLED on this WooCommerce lane
        //    (WC_GOOGLE_PAY_ENABLED = false). Unlike Apple Pay — where Basis
        //    Theory manages the merchant cert and a per-domain .well-known
        //    file is all that's needed — Google Pay registers "the domain
        //    that CALLS the GP API". The platform's design runs Google Pay
        //    through the veyragate.com iframe (/embed/express/google-pay/<id>)
        //    so ONE Google Pay & Wallet Console registration covers every
        //    merchant. THIS classic.js lane would call the GP API INLINE on
        //    the merchant's own origin (e.g. apexpeptidesupply.com), which is
        //    NOT registered → it would need the merchant's own Google sign-up
        //    AND Google's manual production approval (still pending for the
        //    platform account BCR2DN…). It also still uses PAN_ONLY (a live
        //    503 risk). So we ship Apple-Pay-only here; re-enable only after
        //    porting this lane to the veyragate.com GP iframe + Google prod
        //    approval. See memory google-pay-live-2026-06-01.
        if (WC_GOOGLE_PAY_ENABLED && walletCfg.google_pay_enabled && walletCfg.basis_theory_tenant_id) {
    ...
    // An ALWAYS-VISIBLE express mount at the top of the payment area.
    // The Apple Pay button must NOT live inside the Veyra card method's
    // collapsible `.payment_box` (the [data-veyra-gateway] wrapper) —
    // WooCommerce hides that box until the card method is selected, so the
    // shopper had to click "Credit / debit card" before Apple Pay appeared.
    // We mount the wallet block just above `#payment` (above ALL payment
    // methods) so it shows on load regardless of the selected method. It
    // carries `veyragate-pay-fields` so the existing button CSS applies.
    function getExpressMount() {
      var existing = document.querySelector('[data-veyra-express="1"]');
      ...
      mount.setAttribute("data-veyra-express", "1");
      mount.className = "veyragate-pay-express veyragate-pay-fields";
      if (payment && payment.parentNode) {
        payment.parentNode.insertBefore(mount, payment);
        return mount;
      }
      // Fallback only if the payment container is absent: the gateway box.
      if (fields) {
        fields.insertBefore(mount, fields.firstChild);
        return mount;
      }
      return null;
    }
    ...
    function renderWalletBlock(walletCfg) {
      ...
      var mount = getExpressMount();
      if (!mount) return;
      var prior = mount.querySelector('[data-veyra-wallets="1"]');
      if (prior && prior.parentNode) {
        try { prior.parentNode.removeChild(prior); } catch (_e) {}
      }
      renderWalletButtons(mount, walletCfg, onWalletClick);
    }
  }
